...
export default function InstantWinResults({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      ...
      <div 
        className={`
          relative max-w-lg w-full max-h-[90vh] bg-gradient-to-br from-yellow-500/10 via-secondary-800 to-amber-600/10 
          rounded-3xl p-6 sm:p-8 border-2 border-yellow-500/50 shadow-2xl shadow-yellow-500/20
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto min-h-0 mb-4">
          <div className="space-y-3 pr-1">
            {wins.map((win, index) => (
              <div
                key={index}
                className="bg-secondary-700/60 rounded-xl p-3 sm:p-4 border border-yellow-500/30 flex items-center justify-between"
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    <div className="font-semibold text-white text-sm sm:text-base">{win.prizeName}</div>
                    <div className="text-xs sm:text-sm text-gray-400">Ticket #{win.ticketNumber}</div>
                  </div>
                </div>
        ...
        <div className="flex-shrink-0">
          <div className="bg-secondary-900/60 rounded-xl p-3 sm:p-4 space-y-2 mb-4">
            {totalCashWon > 0 && (
              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm sm:text-base">💷 Total Cash Won:</span>
                <span className="text-lg sm:text-xl font-bold text-green-400">£{totalCashWon.toFixed(2)}</span>
              </div>
            )}
            {totalRyderCashWon > 0 && (
              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm sm:text-base">🎫 Total Ryder Cash Won:</span>
                <span className="text-lg sm:text-xl font-bold text-primary-400">£{totalRyderCashWon.toFixed(2)}</span>
              </div>
            )}
          </div>
